 ...
 * `SNMP` (B1), `TLS` (B3), and `SMB` (A7) are direct per-host responses, but Stage C only ever
 * queries already-`CONFIRMED` hosts, so none of the three needs to affect [confidenceOf] either. */
