...
| DNS | Tools → DNS | NXDOMAIN (clean "no records"), a genuinely malformed wire response from a real-world resolver (distinct from NXDOMAIN - confirmed reproducible against `.invalid` TLDs on at least one ISP resolver); custom server that doesn't match any registered server (mismatch flag); blank server field aiming at the first registered server ("Chosen"/"Replied from" rows, keyboard both up and down); no active network at all, e.g. airplane mode (empty "Registered DNS servers" list); Private DNS strict mode against both the system-resolver and custom-server paths - a blank field must fall back to the system resolver with the Private-DNS note, not send cleartext to the registered IP; toggle via `adb shell settings put global private_dns_mode hostname` + `private_dns_specifier`, this is `docs/adr/c-20-private-dns-strict-mode-and-raw-sockets.md`'s pending on-device confirmation; results must stay scrollable clear of the keyboard on a phone with a multi-network "Registered DNS servers" card | design §9.4, §11.3 (degraded-mode labeling), C-19, C-20 |
