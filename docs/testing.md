...
| DNS | Tools → DNS | NXDOMAIN (clean "no records"), a genuinely malformed wire response from a real-world resolver (distinct from NXDOMAIN - confirmed reproducible against `.invalid` TLDs on at least one ISP resolver); custom server that doesn't match any registered server (mismatch flag); no active network at all, e.g. airplane mode (empty "Registered DNS servers" list); Private DNS strict mode against both the system-resolver and custom-server paths - toggle via `adb shell settings put global private_dns_mode hostname` + `private_dns_specifier`, this is `docs/adr/c-20-private-dns-strict-mode-and-raw-sockets.md`'s pending on-device confirmation | design §9.4, §11.3 (degraded-mode labeling), C-19, C-20 |
